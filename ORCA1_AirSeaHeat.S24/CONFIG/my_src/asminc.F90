MODULE asminc
   ...
   LOGICAL, PUBLIC :: ln_icemask = .FALSE. !: Use a sea-ice mask to apply increments
...
CONTAINS
   ...
   SUBROUTINE asm_inc_init
      ...
      NAMELIST/nam_asminc/ ln_bkgwri,                                      &
         ...
         &                 ln_salfix, salfixmin, nn_divdmp, ln_icemask,    &
         &                 ln_temnofreeze
      !!----------------------------------------------------------------------
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nam_asminc in configuration namelist' )
      ...
      IF(lwp) THEN
         ...
         WRITE(numout,*) '      Check T increments do not go below freezing point   ln_temnofreeze = ', ln_temnofreeze
      ENDIF
...
!            WRITE(numout,*) 'asm_inc_init : Assimilation increments valid between dates ', z_inc_dateb,' and ', z_inc_datef
            WRITE(numout,*) 'asm_inc_init : Assimilation increments '
            WRITE(numout,*) '~~~~~~~~~~~~'
         ENDIF
         !
...
!         IF( ln_bkgwri )   CALL asm_bkg_wri( nit000 - 1 )      ! Output background fields
   ...
   SUBROUTINE tra_asm_inc( kt )
      ...
      REAL (wp), DIMENSION(jpi,jpj,jpk) :: zicems ! 3d freezing point values
      !!----------------------------------------------------------------------
      !
            ...
            DO jk = 1, jpkm1
               IF (ln_temnofreeze) THEN
                  ...
                  WHERE(t_bkginc(:,:,jk) > 0.0_wp .OR. &
                     &   tsn(:,:,jk,jp_tem) + tsa(:,:,jk,jp_tem) + t_bkginc(:,:,jk) * zicems(:,:,jk) * wgtiau(it) > fzptnz(:,:,jk) ) 
                     tsa(:,:,jk,jp_tem) = tsa(:,:,jk,jp_tem) + t_bkginc(:,:,jk) * zicems(:,:,jk) * zincwgt  
                  END WHERE
               ELSE
                  tsa(:,:,jk,jp_tem) = tsa(:,:,jk,jp_tem) + t_bkginc(:,:,jk) * zincwgt * zicems(:,:,jk)
               ENDIF
               IF (ln_salfix) THEN
                  ...
                  WHERE(s_bkginc(:,:,jk) > 0.0_wp .OR. &
                     &   tsn(:,:,jk,jp_sal) + tsa(:,:,jk,jp_sal) + s_bkginc(:,:,jk) * zicems(:,:,jk) * wgtiau(it) > salfixmin ) 
                     tsa(:,:,jk,jp_sal) = tsa(:,:,jk,jp_sal) + s_bkginc(:,:,jk) * zicems(:,:,jk) * zincwgt
                  END WHERE
               ELSE
                  tsa(:,:,jk,jp_sal) = tsa(:,:,jk,jp_sal) + s_bkginc(:,:,jk) * zicems(:,:,jk) * zincwgt
               ENDIF
      ...
      ELSEIF ( ln_asmdin ) THEN        ! Direct Initialization
         ...
         IF ( kt == nitdin_r ) THEN
            ...
            IF (ln_temnofreeze) THEN
               ! Do not apply negative increments if the temperature will fall below freezing
               WHERE( t_bkginc(:,:,:) > 0.0_wp .OR. tsn(:,:,:,jp_tem) + t_bkginc(:,:,:) * zicems(:,:,:) > fzptnz(:,:,:) ) 
               !   tsn(:,:,:,jp_tem) = t_bkg(:,:,:) + t_bkginc(:,:,:)   
                  tsn(:,:,:,jp_tem) = tsn(:,:,:,jp_tem) + t_bkginc(:,:,:) * zicems(:,:,:) 
               END WHERE
            ELSE
               tsn(:,:,:,jp_tem) = tsn(:,:,:,jp_tem) + t_bkginc(:,:,:) * zicems(:,:,:)
            ENDIF
            IF (ln_salfix) THEN
               ! Do not apply negative increments if the salinity will fall below a specified
               ! minimum value salfixmin
               WHERE( s_bkginc(:,:,:) > 0.0_wp .OR. tsn(:,:,:,jp_sal) + s_bkginc(:,:,:) * zicems(:,:,:) > salfixmin ) 
                  tsn(:,:,:,jp_sal) = tsn(:,:,:,jp_sal) + s_bkginc(:,:,:) * zicems(:,:,:)
               END WHERE
            ELSE
               tsn(:,:,:,jp_sal) = tsn(:,:,:,jp_sal) + s_bkginc(:,:,:) * zicems(:,:,:)  
            ENDIF
