   ...
   SUBROUTINE ice_stp( kt, ksbc )
      ...
      IF( MOD( kt-1, nn_fsbc ) == 0 ) THEN   ! --- Ice time step --- !
         ...
         !                              ! these calls must remain here for restartability purposes
                                        ...
                                        CALL ice_dmp( kt )            ! Sea-ice damping
         ! SPP perturbations
         IF( ln_stopack .AND. nn_spp_icio > 0 .AND. npti > 0) THEN
             rn_cio2d(:,:) = rn_cio
             CALL spp_gen(kt,rn_cio2d,nn_spp_icio,rn_icio_sd,jk_spp_icio)
         ENDIF
         !
         !------------------------------------------------!
...
!!gm   remark, the ocean-ice stress is not saved in ice diag call above .....  find a solution!!!
   ...
   SUBROUTINE ice_init
      ...
      CALL ctl_opn( numnam_ice_cfg, 'namelist_ice_cfg'//TRIM(c_enss),    'OLD',     'FORMATTED', 'SEQUENTIAL', -1, numout, lwp )
      IF(lwm) CALL ctl_opn( numoni, 'output.namelist.ice'//TRIM(c_enss), 'UNKNOWN', 'FORMATTED', 'SEQUENTIAL', -1, numout, lwp, 1 )
      !
      ...
      !
      CALL ice_dmp_init                ! initialization for damping
      !
