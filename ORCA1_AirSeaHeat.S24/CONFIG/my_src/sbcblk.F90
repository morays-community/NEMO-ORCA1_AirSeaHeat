MODULE sbcblk
...
#if defined key_si3
   USE ice     , ONLY :   u_ice, v_ice, jpl, a_i_b, at_i_b, t_su, rn_cnd_s2d, hfx_err_dif, nn_qtrice, rn_cnd_s
   ...
   USE sbcblk_algo_ncar0    ! => turb_ncar0    : NCAR - CORE (Large & Yeager, 2009) 
   USE cgstoch
   USE sbcblk_corr
...
!!gm  : In principle, yes.
   ...
   LOGICAL, SAVE :: ln_forperturb = .false.
   LOGICAL, SAVE :: ln_forcorrect = .false.
   TYPE(FLD), ALLOCATABLE, DIMENSION(:) ::   sfp  ! structure of input fields (file informations, fields read)
   INTEGER , PARAMETER ::   jppfld   = 6
   INTEGER , PARAMETER ::   jpp_wndi = 1
   INTEGER , PARAMETER ::   jpp_wndj = 2
   INTEGER , PARAMETER ::   jpp_tair = 3
   INTEGER , PARAMETER ::   jpp_qsr  = 4
   INTEGER , PARAMETER ::   jpp_qlw  = 5
   INTEGER , PARAMETER ::   jpp_prec = 6
   INTEGER , SAVE      ::   kn_precip= 0
   !                                             !!! Bulk parameters
   ...
   REAL(wp), ALLOCATABLE, SAVE ::   rn_vfac0(:,:)     ! multiplication factor for ice/ocean velocity in the calculation of wind stress
...
!!gm ref namelist initialize it so remove the setting to false below
   ...
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:) ::   qsrd
   ...
   REAL(wp), ALLOCATABLE, SAVE, PUBLIC :: blk_aux(:,:,:)
...
CONTAINS
   ...
   INTEGER FUNCTION sbc_blk_alloc()
      ...
      ALLOCATE( Cd_atm (jpi,jpj), Ch_atm (jpi,jpj), Ce_atm (jpi,jpj), t_zu(jpi,jpj), q_zu(jpi,jpj), &
         &      cdn_oce(jpi,jpj), chn_oce(jpi,jpj), cen_oce(jpi,jpj), rn_vfac0(jpi,jpj), qsrd(jpi,jpj), &
         &      blk_aux(jpi,jpj,3), STAT=sbc_blk_alloc )
      !
   ...
   SUBROUTINE sbc_blk_init
      ...
      TYPE(FLD_N) ::   snp_wndi, snp_wndj, snp_qsr, snp_qlw, snp_tair, snp_prec
      TYPE(FLD_N), DIMENSION(jppfld) ::   slfp_i               ! array of namelist informations on the fields to read
      NAMELIST/namsbc_blk/ sn_wndi, sn_wndj, sn_humi, sn_qsr, sn_qlw ,                &   ! input fields
         ...
         &                 nn_cgstoch, ln_cgstbia, &
         &                 ln_cgsst,ln_cgssq,ln_cgwnd,ln_cgrho,ln_cgtau,ln_cgflx, &
         &                 ln_forperturb, kn_precip, &
         &                 ln_forcorrect, &
         &                 snp_wndi, snp_wndj, snp_qsr, snp_qlw, snp_tair, snp_prec
      !!---------------------------------------------------------------------
      !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namsbc_blk in configuration namelist' )
      ...
      IF ( ln_forperturb ) THEN
         slfp_i(jpp_wndi) = snp_wndi   ;   slfp_i(jpp_wndj) = snp_wndj
         slfp_i(jpp_qsr ) = snp_qsr    ;   slfp_i(jpp_qlw ) = snp_qlw
         slfp_i(jpp_tair) = snp_tair   ;   slfp_i(jpp_prec) = snp_prec
         ALLOCATE( sfp(jppfld), STAT=ierror )
         IF( ierror > 0 )   CALL ctl_stop( 'STOP', 'sbc_blk_init: unable to allocate sfp structure' )
         CALL fld_fill( sfp, slfp_i, cn_dir, 'sbc_blk_init', 'surface boundary condition -- forcing perturbation', 'namsbc_blk' )
         DO jfpr = 1, jppfld
            IF( TRIM(sfp(jfpr)%clrootname) == 'NOT USED' ) THEN    !--  not used field  --!   (only now allocated and set to zero)
              ALLOCATE( sfp(jfpr)%fnow(jpi,jpj,1) )
              sfp(jfpr)%fnow(:,:,1) = 0._wp
            ELSE                                                  !-- used field --!
              ALLOCATE( sfp(jfpr)%fnow(jpi,jpj,1) )
              IF( slfp_i(jfpr)%ln_tint )   ALLOCATE( sfp(jfpr)%fdta(jpi,jpj,1,2) )
              IF( slfp_i(jfpr)%freqh > 0. .AND. MOD( NINT(3600. * slfp_i(jfpr)%freqh), nn_fsbc * NINT(rdt) ) /= 0 )                      &
               &  CALL ctl_warn( 'sbc_blk_init: sbcmod timestep rdt*nn_fsbc is NOT a submultiple of atmospheric forcing frequency.', &
               &                 '               This is not ideal. You should consider changing either rdt or nn_fsbc value...' )
            ENDIF
         ENDDO
      ...
      IF(lwp) THEN                     !** Control print
         ...
         WRITE(numout,*) '      coarse-graining stochastic algorithm size           nn_cgstoch   = ', nn_cgstoch
         WRITE(numout,*) '      bias to be removed in the coarse-graining algorithm ln_cgstbia   = ', ln_cgstbia
         WRITE(numout,*) '                    coarse-graining algorithm, sst switch ln_cgsst     = ', ln_cgsst  
         WRITE(numout,*) '                    coarse-graining algorithm, ssq switch ln_cgssq     = ', ln_cgssq  
         WRITE(numout,*) '                    coarse-graining algorithm, wnd switch ln_cgwnd     = ', ln_cgwnd  
         WRITE(numout,*) '                    coarse-graining algorithm, rho switch ln_cgrho     = ', ln_cgrho  
         WRITE(numout,*) '                    coarse-graining algorithm, tau switch ln_cgtau     = ', ln_cgtau  
         WRITE(numout,*) '                    coarse-graining algorithm, flx switch ln_cgflx     = ', ln_cgflx  
         WRITE(numout,*) '      Atmospheric input forcing perturbation              ln_forperturb= ', ln_forperturb
         WRITE(numout,*) '      Atmospheric input forcing add/mlt correction        ln_forcorrect= ', ln_forcorrect
         WRITE(numout,*) '                                                          --> fill namsbc_blk_corr '
         WRITE(numout,*) '      Precip horiz offset                                 kn_precip    = ', kn_precip
         !
         WRITE(numout,*)
      ...
      ENDIF
      !
      IF( nn_cgstoch .GT. 1 ) THEN
              CALL CGSTOCH_INIT ( nn_cgstoch )
              CALL turb_ncar0_INIT( ncg_jpi, ncg_jpj, cg_tmask )
      ENDIF
      !
   ...
   SUBROUTINE sbc_blk( kt )
      ...
      REAL(wp) :: ztd, zt0, zps, zzt, zz0
      REAL(wp), PARAMETER :: C1ES=610.78,C2ES=17.269,C3ES=273.16,C4ES=35.86
      INTEGER  :: ji, jj
      REAL(wp) :: z1, z2
      !!---------------------------------------------------------------------
      !
      ...
      IF ( ln_forperturb ) THEN
              !
              CALL fld_read( kt, nn_fsbc, sfp )
              !
              sf(jp_tair)%fnow(:,:,1) = sf(jp_tair)%fnow(:,:,1) + sfp(jpp_tair)%fnow(:,:,1)
              sf(jp_wndi)%fnow(:,:,1) = sf(jp_wndi)%fnow(:,:,1) + sfp(jpp_wndi)%fnow(:,:,1)
              sf(jp_wndj)%fnow(:,:,1) = sf(jp_wndj)%fnow(:,:,1) + sfp(jpp_wndj)%fnow(:,:,1)
              sf(jp_qsr )%fnow(:,:,1) = sf(jp_qsr )%fnow(:,:,1) + sfp(jpp_qsr )%fnow(:,:,1)
              sf(jp_qlw )%fnow(:,:,1) = sf(jp_qlw )%fnow(:,:,1) + sfp(jpp_qlw )%fnow(:,:,1)
              WHERE ( sf(jp_qsr )%fnow(:,:,1) .LT. 0._wp ) sf(jp_qsr )%fnow(:,:,1) = 0._wp
              WHERE ( sf(jp_qlw )%fnow(:,:,1) .LT. 0._wp ) sf(jp_qlw )%fnow(:,:,1) = 0._wp
              IF( kn_precip .EQ. 0 ) THEN
                  sf(jp_prec)%fnow(:,:,1) = sf(jp_prec)%fnow(:,:,1) + sfp(jpp_prec)%fnow(:,:,1)
                  WHERE ( sf(jp_prec)%fnow(:,:,1) .LT. 0._wp ) sf(jp_prec)%fnow(:,:,1) = 0._wp
              ELSE 
                  sfp(jpp_prec)%fnow(:,:,1) = sf(jp_prec)%fnow(:,:,1)
                  DO jj=2,jpj-1
                     DO ji=2,jpi-1
                          IF( kn_precip .EQ. 1 ) sf(jp_prec)%fnow(ji,jj,1) = sfp(jpp_prec)%fnow(ji-1,jj,1)
                          IF( kn_precip .EQ. 2 ) sf(jp_prec)%fnow(ji,jj,1) = sfp(jpp_prec)%fnow(ji+1,jj,1)
                          IF( kn_precip .EQ. 3 ) sf(jp_prec)%fnow(ji,jj,1) = sfp(jpp_prec)%fnow(ji,jj-1,1)
                          IF( kn_precip .EQ. 4 ) sf(jp_prec)%fnow(ji,jj,1) = sfp(jpp_prec)%fnow(ji,jj+1,1)
                     ENDDO
                  ENDDO
                  CALL lbc_lnk( 'sbcblk', sf(jp_prec)%fnow(:,:,1), 'T',  1. )
              ENDIF
              !
              ! Use tangent-linear increment of Tair on specific humidity
              !
              DO jj=1,jpj
                 DO ji=1,jpi
                   zt0 = sf(jp_tair)%fnow(ji,jj,1) - sfp(jpp_tair)%fnow(ji,jj,1)
                   ztd = sf(jp_tair)%fnow(ji,jj,1)
                   zps = sf(jp_slp )%fnow(ji,jj,1)
                   zz0 = 0.622 * C1ES * C2ES * EXP ( C2ES * (zt0 -C3ES) / (zt0 -C4ES) ) / zps
                   zzt = 0.622 * C1ES * C2ES * EXP ( C2ES * (ztd -C3ES) / (ztd -C4ES) ) / zps
                   zzt = zzt - zz0
                   sf(jp_humi)%fnow(ji,jj,1) = MAX(0._wp, sf(jp_humi)%fnow(ji,jj,1) + zzt )
                 ENDDO
              ENDDO
      ENDIF
      !
      IF ( ln_dm2dc ) THEN
          qsrd(:,:) = sbc_dcy( sf(jp_qsr)%fnow(:,:,1) ) * tmask(:,:,1)
      ELSE
          qsrd(:,:) = sf(jp_qsr)%fnow(:,:,1)
      ENDIF
      !
      IF( ln_forcorrect ) THEN

              CALL sbc_blk_corr(kt,sf(jp_wndi)%fnow(:,:,1),sf(jp_wndj)%fnow(:,:,1),sf(jp_slp)%fnow(:,:,1),&
              & sf(jp_tair)%fnow(:,:,1),sf(jp_humi)%fnow(:,:,1),sf(jp_prec)%fnow(:,:,1),sf(jp_snow)%fnow(:,:,1), &
              & qsrd, sf(jp_qlw)%fnow(:,:,1) )
...
!      ENDIF
      ...
      IF( ln_cgstoch ) THEN
             ...
             IF( ln_stopack .AND. (nn_spp_blkd + nn_spp_blkh + nn_spp_blke) > 0 ) &
             & CALL ctl_stop('stopack and cgstoch')

             IF(lwp) write(numout,*) ' Coarse-graining stochastic bulk formulation at timestep', kt

             CALL SCATTER1( ztpot,ztpot0 )
             CALL SCATTER1( sf(jp_humi)%fnow(:,:,1),sf_humi0)
             CALL SCATTER1( sf(jp_slp)%fnow(:,:,1),sf_slp0)

             IF( ln_cgsst ) THEN
                CALL SCATTER2b( zst,zst0,sst_sgst)
             ELSE
                CALL SCATTER1( zst,zst0 )
             ENDIF

             IF( ln_cgssq ) THEN
                zsq0(:,:) = 0.98 * q_sat0( zst0(:,:), sf_slp0, reps0)
                CALL GATHER1(     zsq0,    zsq)
             ELSE
                CALL SCATTER1( zsq,zsq0 )
             ENDIF

             IF( ln_cgwnd ) THEN
                CALL SCATTER2c( wndm,wndm0,ssc_sgst)
             ELSE
                CALL SCATTER1( wndm,wndm0)
             ENDIF

             IF(lwp) write(numout,*) ' turb cor'
             SELECT CASE( nblk )        !==  transfer coefficients  ==!   Cd, Ch, Ce at T-point
             !
             CASE( np_NCAR      )   ;   CALL turb_ncar0   ( rn_zqt, rn_zu, zst0, ztpot0, zsq0, sf_humi0, wndm0, &
                &                                           Cd_atm0, Ch_atm0, Ce_atm0, t_zu0, q_zu0, zU_zu0,      &
                &                                           cdn_oce0, chn_oce0, cen_oce0 )
             ...
             END SELECT
             !
             CALL GATHER1(  Cd_atm0, Cd_atm)
             CALL GATHER1(  Ch_atm0, Ch_atm)
             CALL GATHER1(  Ce_atm0, Ce_atm)
             CALL GATHER1(    t_zu0,   t_zu)
             CALL GATHER1(    q_zu0,   q_zu)
             CALL GATHER1(   zU_zu0,  zU_zu)
             CALL GATHER1( cdn_oce0,cdn_oce)
             CALL GATHER1( chn_oce0,chn_oce)
             CALL GATHER1( cen_oce0,cen_oce)
             !
             IF(ln_cgrho) THEN
                IF( ABS(rn_zu - rn_zqt) > 0.01 ) THEN     ! At zu: (probably useless to remove zrho*grav*rn_zu from SLP...)
                    zrhoa0(:,:) = rho_air0( t_zu0(:,:)              , q_zu0(:,:)              , sf_slp0, R_dry, rctv0 )
                ELSE                                      ! At zt:
                    CALL SCATTER1( sf(jp_tair)%fnow(:,:,1) ,sf_tair0 )
                    zrhoa0(:,:) = rho_air0( sf_tair0, sf_humi0  , sf_slp0, R_dry, rctv0 )
                END IF
                CALL GATHER1(   zrhoa0,  zrhoa)
             ELSE
                IF( ABS(rn_zu - rn_zqt) > 0.01 ) THEN     ! At zu: (probably useless to remove zrho*grav*rn_zu from SLP...)
             ...
             !
             IF(ln_cgtau) THEN
                CALL SCATTER1( zwnd_i, zwnd_i0)
                CALL SCATTER1( zwnd_j, zwnd_j0)
                DO jj = 1, ncg_jpj             ! tau module, i and j component
                  DO ji = 1, ncg_jpi
                    zztmp = zrhoa0(ji,jj)  * zU_zu0(ji,jj) * Cd_atm0(ji,jj)   ! using bulk wind speed
                    taum0 (ji,jj)  = zztmp * wndm0  (ji,jj)
                    zwnd_i0(ji,jj) = zztmp * zwnd_i0(ji,jj)
                    zwnd_j0(ji,jj) = zztmp * zwnd_j0(ji,jj)
                  END DO
                END DO
                CALL GATHER1(  zwnd_i0, zwnd_i)
                CALL GATHER1(  zwnd_j0, zwnd_j)
                CALL GATHER1(    taum0,   taum)
             ELSE
             ...
             IF(ln_cgflx) THEN
                zqla0(:,:) = zrhoa0(:,:) * zU_zu0(:,:) * cg_tmask(:,:)
                IF( ABS( rn_zu - rn_zqt) < 0.01_wp ) THEN
                    !! q_air and t_air are given at 10m (wind reference height)
                    zevap0(:,:) = rn_efac*MAX( 0._wp, zqla0(:,:)*Ce_atm0(:,:)*(zsq0(:,:) - sf_humi0) ) ! Evaporation, using bulk wind speed
                    zqsb0 (:,:) = cp_air0(sf_humi0)*zqla0(:,:)*Ch_atm0(:,:)*(zst0(:,:) - ztpot0(:,:)   )   ! Sensible Heat, using bulk wind speed
                ELSE
                    ...
                    zevap0(:,:) = rn_efac*MAX( 0._wp,  zqla0(:,:)*Ce_atm0(:,:)*(zsq0(:,:) - q_zu0(:,:) ) ) ! Evaporation, using bulk wind speed
                    zqsb0 (:,:) = cp_air0(sf_humi0)*zqla0(:,:)*Ch_atm0(:,:)*(zst0(:,:) - t_zu0(:,:) )   ! Sensible Heat, using bulk wind speed
                ENDIF
                zqla0(:,:) = L_vap0(zst0(:,:)) * zevap0(:,:)     ! Latent Heat flux
                CALL GATHER1(    zqla0,   zqla)
                CALL GATHER1(   zevap0,  zevap)
                CALL GATHER1(    zqsb0,   zqsb)
             ELSE
      ...
      ELSE
             ...
             IF( ln_stopack .AND. nn_spp_blkd > 0 ) THEN
                 CALL spp_gen(kt, Cd_atm, nn_spp_blkd, rn_blkd_sd, jk_spp_blkd )
             ENDIF
             IF( ln_stopack .AND. nn_spp_blkh > 0 ) THEN
                 CALL spp_gen(kt, Ch_atm, nn_spp_blkh, rn_blkh_sd, jk_spp_blkh )
             ENDIF
             IF( ln_stopack .AND. nn_spp_blke > 0 ) THEN
                 CALL spp_gen(kt, Ce_atm, nn_spp_blke, rn_blke_sd, jk_spp_blke )
             ENDIF
             !
             IF( ABS(rn_zu - rn_zqt) > 0.01 ) THEN     ! At zu: (probably useless to remove zrho*grav*rn_zu from SLP...)
                 zrhoa(:,:) = rho_air( t_zu(:,:)              , q_zu(:,:)              , sf(jp_slp)%fnow(:,:,1) )
      ...
      IF(narea==113)THEN
             ...
             DO jj=1,jpj
               IF(mjg(jj)==132) kj=jj
             ENDDO
             where( zevap > 0.0005) zevap = 0.0005
             WRITE(3478,'(3I6,12E12.3)') kt,ki,kj,zst(ki,kj)-rt0,zevap(ki,kj),sf(jp_prec)%fnow(ki,kj,1)*rn_pfac,&
                     & zqlw(ki,kj),zqsb(ki,kj),zqla(ki,kj),sf(jp_snow)%fnow(ki,kj,1)*rn_pfac,qns(ki,kj),qsr(ki,kj),&
                     & Ce_atm(ki,kj),zsq(ki,kj),sf(jp_humi)%fnow(ki,kj,1)
             ! zqla(:,:)*Ce_atm(:,:)*(zsq(:,:) - sf(jp_humi)%fnow(:,:,1))
      ENDIF
#endif
      !
      ! These are not output by ice routines, needs to be done here
   ...
   SUBROUTINE blk_ice_tau
      ...
      IF( ln_stopack .AND. nn_spp_blkd > 0 ) THEN
          CALL spp_gen( 0, Cd_atm, nn_spp_blkd, rn_blkd_sd, jk_spp_blkd )
      ENDIF
      IF( ln_stopack .AND. nn_spp_blkh > 0 ) THEN
          CALL spp_gen( 0, Ch_atm, nn_spp_blkh, rn_blkh_sd, jk_spp_blkh )
      ENDIF
      IF( ln_stopack .AND. nn_spp_blke > 0 ) THEN
          CALL spp_gen( 0, Ce_atm, nn_spp_blke, rn_blke_sd, jk_spp_blke )
      ENDIF
   ...
   SUBROUTINE blk_ice_qcn( ld_virtual_itd, ptsu, ptb, phs, phi )
      ...
      IF( ld_virtual_itd ) THEN
         ...
         DO jl = 1, jpl                
            DO jj = 1 , jpj
               DO ji = 1, jpi
                  zhe = ( rn_cnd_s2d(ji,jj) * phi(ji,jj,jl) + rcnd_i * phs(ji,jj,jl) ) /  ( rn_cnd_s2d(ji,jj) + rcnd_i )          ! Effective thickness
      ...
      DO jl = 1, jpl
         DO jj = 1 , jpj
            DO ji = 1, jpi
               !                    
               zkeff_h = rcnd_i * rn_cnd_s2d(ji,jj) * zgfac(ji,jj,jl) / &                                    ! Effective conductivity of the snow-ice system divided by thickness
                  &      ( rcnd_i * phs(ji,jj,jl) + rn_cnd_s2d(ji,jj) * MAX( 0.01, phi(ji,jj,jl) ) )
