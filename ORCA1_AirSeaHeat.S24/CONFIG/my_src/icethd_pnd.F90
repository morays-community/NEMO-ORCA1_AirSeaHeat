   ...
   SUBROUTINE pnd_LEV
      ...
      DO ji = 1, npti
         ...
         ELSE                                                         ! Case ice thickness >= rn_himin !
            ...
            zdv_flush   = -zperm * rau0 * grav * zhp * rdt_ice / (zvisc * h_i_1d(ji)) * a_ip_1d(ji) * rn_pnd_flush1d(ji)
...
!!$            diag_dvpn_drn_1d(ji) = diag_dvpn_drn_1d(ji) + rhow *   zdv_flush             * r1_rdtice   ! < 0, drainage
   ...
   SUBROUTINE ice_thd_pnd_init 
      ...
      NAMELIST/namthd_pnd/  ln_pnd, ln_pnd_LEV , rn_apnd_min, rn_apnd_max, &
         ...
         &                          ln_pnd_lids, ln_pnd_alb, rn_pnd_flush
      !!-------------------------------------------------------------------
      !
      rn_pnd_flush = 0.1
      !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namthd_pnd in configuration namelist' )
      ...
      END SELECT
      !
      IF( rn_pnd_flush < 0.01 .OR. rn_pnd_flush > 1.) CALL ctl_stop( 'ice_thd_pnd_init: rn_pnd_flush NOT SET' )
      ALLOCATE (rn_pnd_flush2d(jpi,jpj)) ; rn_pnd_flush2d(:,:) = rn_pnd_flush
      !
