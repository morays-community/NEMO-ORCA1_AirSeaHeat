...
!! INTEGER,  PARAMETER  ::  DP=SELECTED_REAL_KIND( 12, 60 )
   ...
   ! Seed initialization for standard fortran
   LOGICAL, SAVE :: LLINIS = .FALSE.
...
subroutine random_stduniformv(u)
   implicit none
...
subroutine random_stdnormalv(x)
   implicit none
   real(WP),intent(inout) :: x(:)
   real(WP),parameter :: pi=3.14159265359_wp
   real(WP), allocatable :: u1(:),u2(:)
   allocate(u1(size(x)),u2(size(x)))
   call random_stduniformv(u1)
   call random_stduniformv(u2)
   x = sqrt(-2*log(u1))*cos(2*pi*u2)
   deallocate (u1,u2)
...
REAL(WP) :: RNORMV(N)
INTEGER :: K
IF(.NOT. LLINIS) CALL INISEED
DO K=1,N
   CALL random_stdnormal( RNORMV(K) )
ENDDO
END FUNCTION RNORMV
...
! Through out first 100
CALL RANDOM_NUMBER(ZWASH(100))
LLINIS = .TRUE.
END SUBROUTINE INISEED
