...
# endif
      ...
      IF( ln_stopack ) CALL stopack_pert( kstp )     ! STOPACK perturbation
      IF( ln_skeb  )   CALL skeb_comp( kstp )        ! SKEB perturbations
      ...
      IF( ln_skeb .AND. ln_skeb_apply .and. ABS( nn_skst ) .eq. 3) CALL skeb_apply ( kstp )
                            CALL dyn_zdf    ( kstp )  ! vertical diffusion
      IF( ln_dynspg_ts ) THEN                         ! vertical scale factors and vertical velocity need to be updated
                            CALL wzv        ( kstp )              ! now cross-level velocity 
         IF( ln_zad_Aimp )  CALL wAimp      ( kstp )  ! Adaptive-implicit vertical advection partitioning
      ENDIF
      IF( ln_skeb .AND. ln_skeb_apply .and. ABS( nn_skst ) .eq. 1) CALL skeb_apply ( kstp )
      ...
      IF(  ln_meft   )   CALL tra_meft      ( kstp )  ! apply model error forcing term
      ...
      IF( lrst_oce .AND. ln_stopack)   &
                         CALL stopack_rst( kstp, 'WRITE' )
...
!!jc2: dynnxt must be the latest call. e3t_b are indeed updated in that routine
      ...
      IF( ln_skeb .AND. ln_skeb_apply .and. ABS(nn_skst) .eq. 2) CALL skeb_apply ( kstp )
