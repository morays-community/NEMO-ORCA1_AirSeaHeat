MODULE sbcrnf
   ...
   LOGICAL , PUBLIC ::   ln_addrnf= .false.   !: runoffs recieved from oasis
...
CONTAINS
   ...
   SUBROUTINE sbc_rnf( kt )
      ...
      IF( MOD( kt - 1, nn_fsbc ) == 0 ) THEN
         !
         IF( .NOT. l_rnfcpl ) THEN
             IF ( ln_addrnf ) THEN  ! Add runoff from file to that from coupler
                 ! zm1 = maxval ( abs ( rnf(:,:)*tmask(:,:,1) ) )
                 ! zm2 = maxval ( abs ( rn_rfact * ( sf_rnf(1)%fnow(:,:,1) ) * tmask(:,:,1) ) )
                 ! IF(lk_mpp) CALL mpp_max( "sbcrnf", zm1 )
                 ! IF(lk_mpp) CALL mpp_max( "sbcrnf", zm2 )
                 WHERE ( rnf(:,:) .GT. 1.e-02_wp ) rnf(:,:) = 1.e-02_wp
                 rnf(:,:) = rnf(:,:)*tmask(:,:,1) + rn_rfact * ( sf_rnf(1)%fnow(:,:,1) ) * tmask(:,:,1)  ! updated runoff value at time step kt
                 ! zm3 = maxval ( abs ( rnf(:,:) ))
                 ! IF(lk_mpp) CALL mpp_max( "sbcrnf", zm3 )
                 ! IF(lwp) WRITE(numout,*) ' Runoff at t = ',kt, ' : ',zm1, zm2, zm3
             ELSE
         ...
         IF( ln_rnf_tem ) THEN                                       ! use runoffs temperature data
            ...
            WHERE( sf_t_rnf(1)%fnow(:,:,1) < ztfrz )             ! if missing data value use SST as runoffs temperature
   ...
   SUBROUTINE sbc_rnf_init
      ...
      NAMELIST/namsbc_rnf/ cn_dir            , ln_rnf_depth, ln_rnf_tem, ln_rnf_sal, ln_rnf_icb,   &
         ...
         &                 ln_rnf_depth_ini  , rn_dep_max  , rn_rnf_max, nn_rnf_depth_file, ln_addrnf
      !!----------------------------------------------------------------------
      !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namsbc_rnf in configuration namelist' )
      ...
      IF(lwp) THEN
         ...
         WRITE(numout,*) '      add runoff in case of coupled config       ln_addrnf    = ', ln_addrnf
      ENDIF
      !
      IF( ln_addrnf ) THEN
              l_rnfcpl = .false.
              IF(lwp) WRITE(numout,*) ' ln_addrnf TRUE : forcing l_rnfcpl to FALSE'
      ENDIF
      !
      !                                   ! ==================
      !                                   !   Type of runoff
