...
CONTAINS
   ...
   SUBROUTINE ice_alb( pt_su, ph_ice, ph_snw, ld_pnd_alb, pafrac_pnd, ph_pnd, pcloud_fra, palb_ice )
      ...
      !
      REAL(wp), DIMENSION(jpi,jpj) :: alb_sdry, alb_smlt
      !!---------------------------------------------------------------------
      !
      ...
      !
      alb_sdry(:,:) = rn_alb_sdry       ;    alb_smlt(:,:) = rn_alb_smlt
      IF(ln_stopack .AND. nn_spp_icealb > 0 .AND. nn_albpert < 0 ) THEN
         IF( nn_albpert .EQ. -1) THEN
               CALL spp_gen(1, alb_sdry(:,:), nn_spp_icealb,rn_icealb_sd,jk_spp_alb,jl)
               CALL spp_gen(1, alb_smlt(:,:), nn_spp_icealb,rn_icealb_sd,jk_spp_alb,jl)
         ELSEIF( nn_albpert .EQ. -2) THEN
            DO jl=1,jpl
               CALL spp_gen(1, za_s_fra(:,:,jl),nn_spp_icealb,rn_icealb_sd,jk_spp_alb,jl)
            ENDDO
         ELSE
            CALL ctl_stop('STOP','nn_albpert option not recognized')
         ENDIF
      ENDIF
      !
      DO jl = 1, jpl
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               IF( pt_su(ji,jj,jl) < rt0 ) THEN
                  zalb_snw = alb_sdry(ji,jj) - ( alb_sdry(ji,jj) - zalb_ice ) * EXP( - ph_snw(ji,jj,jl) * z1_c3 )
               ELSE
                  zalb_snw = alb_smlt(ji,jj) - ( alb_smlt(ji,jj) - zalb_ice ) * EXP( - ph_snw(ji,jj,jl) * z1_c4 )
               ENDIF
               !                       !--- Ponded ice albedo
      ...
      !
      IF(ln_stopack .AND. nn_spp_icealb > 0 .AND. nn_albpert >= 0 ) THEN
         IF( nn_albpert .EQ. 0 ) THEN
            DO jl=1,jpl
               CALL spp_gen(1, palb_ice(:,:,jl),nn_spp_icealb,rn_icealb_sd,jk_spp_alb,jl)
            ENDDO
         ELSEIF( nn_albpert .EQ. 1 ) THEN
            CALL spp_gen(1, palb_ice(:,:,1),nn_spp_icealb,rn_icealb_sd,jk_spp_alb,jl)
         ELSEIF( nn_albpert .EQ. 2 ) THEN
            CALL spp_gen(1, palb_ice(:,:,jpl),nn_spp_icealb,rn_icealb_sd,jk_spp_alb,jl)
         ELSE
            CALL ctl_stop('STOP','nn_albpert option not recognized')
         ENDIF
      ENDIF
