MODULE sbcblk_algo_ncar0
   ...
   PUBLIC :: TURB_NCAR0, turb_ncar0_INIT  ! called by sbcblk.F90
   ...
   INTEGER , SAVE :: n_jpi,n_jpj
   REAL(wp), ALLOCATABLE :: tmask0(:,:)

   !!----------------------------------------------------------------------
CONTAINS

   SUBROUTINE turb_ncar0_INIT(i1,i2,rmin)
           INTEGER, INTENT(IN) :: i1,i2
           REAL(wp), INTENT(IN) :: rmin(i1,i2)
           n_jpi = i1
           n_jpj = i2
           ALLOCATE(tmask0(i1,i2))
           tmask0 = rmin
           IF(lwp) write(numout,*) ' turb_ncar0_INIT ==',n_jpi,n_jpj
   END SUBROUTINE turb_ncar0_INIT

   SUBROUTINE turb_ncar0( zt, zu, sst, t_zt, ssq, q_zt, U_zu, &
      ...
      REAL(wp), INTENT(in   ), DIMENSION(n_jpi,n_jpj) ::   sst      ! sea surface temperature                [Kelvin]
      REAL(wp), INTENT(in   ), DIMENSION(n_jpi,n_jpj) ::   t_zt     ! potential air temperature              [Kelvin]
      REAL(wp), INTENT(in   ), DIMENSION(n_jpi,n_jpj) ::   ssq      ! sea surface specific humidity           [kg/kg]
      REAL(wp), INTENT(in   ), DIMENSION(n_jpi,n_jpj) ::   q_zt     ! specific air humidity at zt             [kg/kg]
      REAL(wp), INTENT(in   ), DIMENSION(n_jpi,n_jpj) ::   U_zu     ! relative wind module at zu                [m/s]
      REAL(wp), INTENT(  out), DIMENSION(n_jpi,n_jpj) ::   Cd       ! transfer coefficient for momentum         (tau)
      REAL(wp), INTENT(  out), DIMENSION(n_jpi,n_jpj) ::   Ch       ! transfer coefficient for sensible heat (Q_sens)
      REAL(wp), INTENT(  out), DIMENSION(n_jpi,n_jpj) ::   Ce       ! transfert coefficient for evaporation   (Q_lat)
      REAL(wp), INTENT(  out), DIMENSION(n_jpi,n_jpj) ::   t_zu     ! pot. air temp. adjusted at zu               [K]
      REAL(wp), INTENT(  out), DIMENSION(n_jpi,n_jpj) ::   q_zu     ! spec. humidity adjusted at zu           [kg/kg]
      REAL(wp), INTENT(  out), DIMENSION(n_jpi,n_jpj) ::   Ub    ! bulk wind speed at zu                     [m/s]
      REAL(wp), INTENT(  out), DIMENSION(n_jpi,n_jpj) ::   CdN, ChN, CeN ! neutral transfer coefficients
      !
      INTEGER :: j_itt
      LOGICAL :: l_zt_equal_zu = .FALSE.      ! if q and t are given at same height as U
      !
      REAL(wp), DIMENSION(n_jpi,n_jpj) ::   Cx_n10        ! 10m neutral latent/sensible coefficient
      REAL(wp), DIMENSION(n_jpi,n_jpj) ::   sqrtCdN10   ! root square of Cd_n10
      REAL(wp), DIMENSION(n_jpi,n_jpj) ::   zeta_u        ! stability parameter at height zu
      REAL(wp), DIMENSION(n_jpi,n_jpj) ::   zpsi_h_u
      REAL(wp), DIMENSION(n_jpi,n_jpj) ::   ztmp0, ztmp1, ztmp2
      REAL(wp), DIMENSION(n_jpi,n_jpj) ::   sqrtCd
      !!----------------------------------------------------------------------------------
      ...
      IF( ln_cdgw ) THEN      ! wave drag case
         CdN = MAX( cdn_wave(:,:) + rsmall * ( 1._wp - tmask0(:,:) ) , 0.1E-3_wp )
      ELSE
         CdN = CD_N10_NCAR( Ub )
      ENDIF
      sqrtCdN10 = SQRT( CdN )
   ...
   FUNCTION CD_N10_NCAR( pw10 )
      ...
      !!----------------------------------------------------------------------------------
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: pw10           ! scalar wind speed at 10m (m/s)
      REAL(wp), DIMENSION(n_jpi,n_jpj)             :: CD_N10_NCAR
      !
   ...
   FUNCTION CH_N10_NCAR( psqrtcdn10 , pstab )
      ...
      !!----------------------------------------------------------------------------------
      REAL(wp), DIMENSION(n_jpi,n_jpj)             :: ch_n10_ncar
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: psqrtcdn10 ! sqrt( CdN10 )
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: pstab      ! stable ABL => 1 / unstable ABL => 0
      !!----------------------------------------------------------------------------------
      !
   ...
   FUNCTION CE_N10_NCAR( psqrtcdn10 )
      ...
      !!----------------------------------------------------------------------------------
      REAL(wp), DIMENSION(n_jpi,n_jpj)             :: ce_n10_ncar
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: psqrtcdn10 ! sqrt( CdN10 )
      !!----------------------------------------------------------------------------------
   ...
   FUNCTION psi_m_ncar( pzeta )
      ...
      !! ** Author: L. Brodeau, June 2016 / AeroBulk (https://github.com/brodeau/aerobulk/)
      !!----------------------------------------------------------------------------------
      REAL(wp), DIMENSION(n_jpi,n_jpj) :: psi_m_ncar
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: pzeta
      !
      INTEGER  ::   ji, jj    ! dummy loop indices
   ...
   FUNCTION psi_h_ncar( pzeta )
      ...
      !! ** Author: L. Brodeau, June 2016 / AeroBulk (https://github.com/brodeau/aerobulk/)
      !!----------------------------------------------------------------------------------
      REAL(wp), DIMENSION(n_jpi,n_jpj) :: psi_h_ncar
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: pzeta
      !
      INTEGER  ::   ji, jj     ! dummy loop indices
   ...
   FUNCTION UN10_from_CD( pzu, pUb, pCd, ppsi )
      ...
      !!----------------------------------------------------------------------------------
      REAL(wp), DIMENSION(n_jpi,n_jpj)             :: UN10_from_CD  !: [m/s]
      REAL(wp),                     INTENT(in) :: pzu  !: measurement heigh of bulk wind speed
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: pUb  !: bulk wind speed at height pzu m   [m/s]
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: pCd  !: drag coefficient
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: ppsi !: "Psi_m(pzu/L)" stability correction profile for momentum []
      !!----------------------------------------------------------------------------------
   ...
   FUNCTION One_on_L( ptha, pqa, pus, pts, pqs )
      ...
      !!         (https://github.com/brodeau/aerobulk/)
      !!------------------------------------------------------------------------
      REAL(wp), DIMENSION(n_jpi,n_jpj)             :: One_on_L     !: 1./(Obukhov length) [m^-1]
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: ptha         !: reference potential temperature of air [K]
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: pqa          !: reference specific humidity of air   [kg/kg]
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: pus          !: u*: friction velocity [m/s]
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: pts, pqs     !: \theta* and q* friction aka turb. scales for temp. and spec. hum.
      !
      INTEGER  ::   ji, jj         ! dummy loop indices
   ...
   FUNCTION z0_from_Cd( pzu, pCd,  ppsi )
      REAL(wp), DIMENSION(n_jpi,n_jpj) :: z0_from_Cd        !: roughness length [m]
      REAL(wp)                    , INTENT(in) :: pzu   !: reference height zu [m]
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: pCd   !: (neutral or non-neutral) drag coefficient []
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in), OPTIONAL :: ppsi !: "Psi_m(pzu/L)" stability correction profile for momentum []
      !!
   ...
   FUNCTION virt_temp( pta, pqa )
      REAL(wp), DIMENSION(n_jpi,n_jpj)             :: virt_temp !: virtual temperature [K]
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: pta !: absolute or potential air temperature [K]
      REAL(wp), DIMENSION(n_jpi,n_jpj), INTENT(in) :: pqa !: specific humidity of air   [kg/kg]
      virt_temp(:,:) = pta(:,:) * (1._wp + rctv0*pqa(:,:))
   END FUNCTION virt_temp

   !!======================================================================
END MODULE sbcblk_algo_ncar0
