MODULE zdfphy
   ...
   REAL(wp), ALLOCATABLE, SAVE :: rn_avt_rnf0(:,:)
...
!!gm
   ...
   SUBROUTINE zdf_phy( kt )
      ...
      !
      IF( ln_stopack .AND. nn_spp_avtb > 0 ) THEN
        avtb_2d(:,:) = 1._wp
        IF( nn_havtb == 1 ) THEN
             WHERE(-15. <= gphit .AND. gphit < -5 )   avtb_2d = (1.  - 0.09 * (gphit + 15.))
             WHERE( -5. <= gphit .AND. gphit <  5 )   avtb_2d =  0.1
             WHERE(  5. <= gphit .AND. gphit < 15 )   avtb_2d = (0.1 + 0.09 * (gphit -  5.))
        ENDIF
        CALL spp_gen(kt, avtb_2d, nn_spp_avtb, rn_avtb_sd, jk_spp_avtb )
      ENDIF
      !
...
!!gm         avm(2:jpim1,2:jpjm1,1:jpkm1) = rn_avm0 * wmask(2:jpim1,2:jpjm1,1:jpkm1)
      ...
      IF( ln_rnf_mouth ) THEN                   !* increase diffusivity at rivers mouths
         IF( .NOT. ALLOCATED( rn_avt_rnf0 )) ALLOCATE(rn_avt_rnf0(jpi,jpj)) 
         rn_avt_rnf0 = rn_avt_rnf
         IF ( ln_stopack .AND. nn_spp_arnf .GT. 0 ) THEN
              CALL spp_gen( kt, rn_avt_rnf0,nn_spp_arnf,rn_arnf_sd,jk_spp_arnf)
         ENDIF
         DO jk = 2, nkrnf
            avt(:,:,jk) = avt(:,:,jk) + 2._wp * rn_avt_rnf0(:,:) * rnfmsk(:,:) * wmask(:,:,jk)
         END DO
      ENDIF
      !
