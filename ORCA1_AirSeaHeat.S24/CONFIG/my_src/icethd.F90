...
!!gm list trop longue ==>>> why not passage en argument d'appel ?
   ...
   USE timing         ! Timing
   USE stopack        ! Stochastic physics scheme
...
CONTAINS
   ...
   SUBROUTINE ice_thd( kt )
      ...
      REAL(wp), DIMENSION(jpi,jpj) ::   zch2d
      !
      !!-------------------------------------------------------------------
      ! controls
      ...
      ! SPP perturbations
      IF( ln_stopack .AND. nn_spp_icnds > 0 .AND. npti > 0) THEN
         rn_cnd_s2d(:,:) = rn_cnd_s
         CALL spp_gen(kt,rn_cnd_s2d,nn_spp_icnds,rn_icnds_sd,jk_spp_icnds)
      ENDIF

      IF( ln_stopack .AND. nn_spp_ioiht > 0 .AND. npti > 0) THEN
         CALL spp_gen(kt,zch2d,nn_spp_ioiht,rn_ioiht_sd,jk_spp_ioiht)
      ENDIF

      IF( ln_stopack .AND. nn_spp_itmfl > 0 .AND. npti > 0) THEN
         rn_time_fl2d(:,:) = rn_time_fl
         CALL spp_gen(kt,rn_time_fl2d,nn_spp_itmfl,rn_itmfl_sd,jk_spp_itmfl)
      ENDIF

      IF( ln_stopack .AND. nn_spp_itmgd > 0 .AND. npti > 0) THEN
         rn_time_gd2d(:,:) = rn_time_gd
         CALL spp_gen(kt,rn_time_gd2d,nn_spp_itmgd,rn_itmgd_sd,jk_spp_itmgd)
      ENDIF

      IF( ln_stopack .AND. nn_spp_ipndfl > 0 .AND. npti > 0) THEN
         rn_pnd_flush2d(:,:) = rn_pnd_flush
         CALL spp_gen(kt,rn_pnd_flush2d,nn_spp_ipndfl,rn_ipndfl_sd,jk_spp_ipndfl)
      ENDIF

      IF( ln_stopack .AND. nn_spp_ihin > 0) THEN
         rn_hinew2d(:,:) = rn_hinew
         CALL spp_gen(kt,rn_hinew2d,nn_spp_ihin,rn_ihin_sd,jk_spp_ihin)
         WHERE( rn_hinew2d(:,:) < rn_himin ) rn_hinew2d(:,:) = rn_himin
      ENDIF
      ...
      IF( ln_icedyn ) THEN
         ...
         DO jj = 2, jpjm1 
            DO ji = fs_2, fs_jpim1
               zfric(ji,jj) = rn_cio2d(ji,jj) * ( 0.5_wp *  &
      ...
      DO jj = 1, jpj
         DO ji = 1, jpi
            ...
            qsb_ice_bot(ji,jj) = rswitch * rau0 * rcp * zch2d(ji,jj) * zfric_u * ( ( sst_m(ji,jj) + rt0 ) - t_bo(ji,jj) )
      ...
      DO jl = 1, jpl
         ...
         IF( npti > 0 ) THEN  ! If there is no ice, do nothing.
            !                                                                
            IF( ln_stopack .AND. ( ln_sppt_icezdf .OR. ln_sppt_icelat ) ) CALL sppt_ice( 1, jl)
            !                                      
            ...
            !                                                       ! --- & Move to 2D arrays --- !
            !                                      
            IF( ln_stopack .AND. ( ln_sppt_icezdf .OR. ln_sppt_icelat ) ) CALL sppt_ice( 2, jl)
         ENDIF
         !
      END DO
      !
   ...
   SUBROUTINE ice_thd_1d2d( kl, kn )
      ...
      CASE( 1 )            !==  from 2D to 1D  ==!
         ...
         !
         ! For SPP
         CALL tab_2d_1d( npti, nptidx(1:npti), rn_cnd_s1d(1:npti)    , rn_cnd_s2d )
         CALL tab_2d_1d( npti, nptidx(1:npti), rn_pnd_flush1d(1:npti), rn_pnd_flush2d)
         CALL tab_2d_1d( npti, nptidx(1:npti), rn_time_fl1d(1:npti)  , rn_time_fl2d)
         CALL tab_2d_1d( npti, nptidx(1:npti), rn_time_gd1d(1:npti)  , rn_time_gd2d)
         !
         ! --- Change units of e_i, e_s from J/m2 to J/m3 --- !
         DO jk = 1, nlay_i
