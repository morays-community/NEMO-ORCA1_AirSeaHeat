      ...
      DO jter = 1 , nn_nevp                           !    loop over jter    !
         ...
         IF( MOD(jter,2) == 0 ) THEN ! even iterations
            ...
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1
                  !                 !--- tau_io/(v_oce - v_ice)
                  zTauO = zaV(ji,jj) * rau0 * rn_cio2d(ji,jj) * SQRT( ( v_ice (ji,jj) - v_oce (ji,jj) ) * ( v_ice (ji,jj) - v_oce (ji,jj) )  &
...
!!            CALL agrif_interp_ice( 'V', jter, nn_nevp )
            ...
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1          
                  !                 !--- tau_io/(u_oce - u_ice)
                  zTauO = zaU(ji,jj) * rau0 * rn_cio2d(ji,jj) * SQRT( ( u_ice (ji,jj) - u_oce (ji,jj) ) * ( u_ice (ji,jj) - u_oce (ji,jj) )  &
