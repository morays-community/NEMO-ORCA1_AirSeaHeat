MODULE icerst
   ...
   USE sbc_ice , ONLY : alb_ice_tot                 ! SAS ss[st]_m init
...
CONTAINS
   ...
   SUBROUTINE ice_rst_write( kt )
      ...
      ! 
      CALL iom_rstput( iter, nitrst, numriw, 'O_AlbIce' , alb_ice_tot)
      CALL iom_rstput( iter, nitrst, numriw, 'O_TepIce' , tm_su )
      CALL iom_rstput( iter, nitrst, numriw, 'OIceFrc'  , at_i )
      CALL iom_rstput( iter, nitrst, numriw, 'O_SSTSST' , tsn(:,:,1,jp_tem) + 273.15 )
      ! 
      ! Snow enthalpy
      DO jk = 1, nlay_s 
