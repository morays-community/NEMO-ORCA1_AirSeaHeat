...
CONTAINS
   ...
   SUBROUTINE ice_dyn_rdgrft( kt )
      ...
      REAL(wp), DIMENSION(jpi, jpj) :: zworka1, zworka2
      REAL(wp), DIMENSION(jpij) ::   zworka1i,zworka2i
      !
      ...
      IF( ln_stopack .AND. nn_spp_icraft > 0 .AND. npti > 0) THEN
         zworka1(:,:) = rn_craft
         CALL spp_gen(kt,zworka1,nn_spp_icraft,rn_icraft_sd,jk_spp_icraft)
      ENDIF

      IF( ln_stopack .AND. nn_spp_icsrdg > 0 .AND. npti > 0) THEN
         zworka2(:,:) = rn_csrdg
         CALL spp_gen(kt,zworka2,nn_spp_icsrdg,rn_icsrdg_sd,jk_spp_icsrdg)
      ENDIF
      ...
      IF( npti > 0 ) THEN
         ...
         IF( ln_stopack .AND. nn_spp_icsrdg > 0 ) THEN

            CALL tab_2d_1d( npti, nptidx(1:npti), zworka2i(1:npti)   , zworka2 )

            DO ji = 1, jpij
               ! closing_net = rate at which open water area is removed + ice area removed by ridging 
               !                                                        - ice area added in new ridges
               closing_net(ji) = zworka2i(ji) * 0.5_wp * ( zdelt(ji) - ABS( zdivu(ji) ) ) - MIN( zdivu(ji), 0._wp )
               !
         ...
         ! 2) Identify grid cells with ridging
         !------------------------------------
         IF( ln_stopack .AND. nn_spp_icraft > 0 ) CALL tab_2d_1d( npti, nptidx(1:npti), zworka1i(1:npti), zworka1 )
         CALL rdgrft_prep( a_i_2d, v_i_2d, ato_i_1d, closing_net, zworka1i )
   ...
   SUBROUTINE rdgrft_prep( pa_i, pv_i, pato_i, pclosing_net, zwa)
      !!-------------------------------------------------------------------
      !!                ***  ROUTINE rdgrft_prep ***
      !!
      ...
      REAL(wp), DIMENSION(:),INTENT(in) ::   zwa
      !!
      ...
      IF( ln_rafting .AND. ln_ridging ) THEN             !- ridging & rafting
         IF( ln_stopack .AND. nn_spp_icraft > 0) THEN
            DO jl = 1, jpl
               DO ji = 1, npti
                  aridge(ji,jl) = ( 1._wp + TANH ( zwa(ji) * ( zhi(ji,jl) - rn_hraft ) ) ) * 0.5_wp * apartf(ji,jl)
   ...
   SUBROUTINE ice_strength
      ...
      !                              !--------------------------------------------------!
      IF(ln_stopack .AND. nn_spp_icestr > 0 ) THEN 
         zworka(:,:) = rn_pstar
         CALL spp_gen(0 ,zworka,nn_spp_icestr,rn_icestr_sd,jk_spp_rhg)
         strength(:,:) = zworka(:,:)*SUM( v_i(:,:,:), dim=3 )*EXP( -rn_crhg * ( 1._wp - SUM( a_i(:,:,:), dim=3 ) ) )
      ELSE
