...
!!   !                                   ! Other namelist parameters
   ...
   REAL(wp) ::   rpref = 101325._wp   ! reference atmospheric pressure[N/m2] 
...
!!       WRITE(numout,*)'      transmitted solar thru sea-ice  = ', TRIM(sn_rcv_qtrice%cldes), ' (', TRIM(sn_rcv_qtrice%clcat), ')'
      ...
      IF( nn_skin_sst .NE. 0 .AND. .NOT. ln_diurnal )  CALL ctl_stop( 'STOP', 'sbc_cpl : skin_sst is TRUE but skin sst scheme is not enabled' )
      ...
      !                                                      ! ------------------------- !
      srcv(ntypsbc,jpr_mslp)%clname = 'O_MSLP'
      IF( TRIM(sn_rcv_mslp%cldes  ) == 'coupled' ) THEN
           srcv(ntypsbc,jpr_mslp)%laction = .TRUE.
           ln_apr_cpl = .TRUE.
      ENDIF
      !
      !                                                      ! --------------------------------- !
