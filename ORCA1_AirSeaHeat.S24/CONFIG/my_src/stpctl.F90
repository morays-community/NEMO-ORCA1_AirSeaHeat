...
CONTAINS
   ...
   SUBROUTINE stp_ctl( kt )
      ...
      IF( kt == nit000 ) THEN
         ...
         ENDIF
         !                                ! open time.step file
         IF( lwm ) CALL ctl_opn( numstp, 'time.step'//TRIM(c_enss), 'REPLACE', 'FORMATTED', 'SEQUENTIAL', -1, numout, lwp, narea )
         !                                ! open run.stat file(s) at start whatever
         !                                ! the value of sn_cfctl%ptimincr
         IF( ll_wrtruns ) THEN
            CALL ctl_opn( numrun, 'run.stat'//TRIM(c_enss), 'REPLACE', 'FORMATTED', 'SEQUENTIAL', -1, numout, lwp, narea )
            clname = 'run.stat.nc'
      ...
      IF(   zmax(1) >   20._wp .OR.   &                    ! too large sea surface height ( > 20 m )
         ...
         ENDIF
         !
         CALL dia_wri_state( 'output.abort'//TRIM(c_enss) )    ! create an output.abort file
         !
