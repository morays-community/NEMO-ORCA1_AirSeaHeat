...
!!gm case neuler  not taken into account....
      ...
      CASE( np_RGB , np_RGBc )         !==  R-G-B fluxes  ==!
         ...
         DO jk = 2, nksr+1                   !* interior equi-partition in R-G-B depending of vertical profile of Chl
            ...
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1
                  zc0 = ze0(ji,jj,jk-1) * EXP( - e3t_n(ji,jj,jk-1) * xsi0r(ji,jj))
      ...
      CASE( np_2BD  )            !==  2-bands fluxes  ==!
         ...
         DO jk = 1, nksr                          ! solar heat absorbed at T-point in the top 400m 
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1
                  zc0 = zz0 * EXP( -gdepw_n(ji,jj,jk  )*xsi0r(ji,jj) ) + zz1 * EXP( -gdepw_n(ji,jj,jk  )*xsi1r )
                  zc1 = zz0 * EXP( -gdepw_n(ji,jj,jk+1)*xsi0r(ji,jj) ) + zz1 * EXP( -gdepw_n(ji,jj,jk+1)*xsi1r )
