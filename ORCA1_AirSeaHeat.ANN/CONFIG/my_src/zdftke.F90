MODULE zdftke
   ...
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   ztmp0
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   rn_ediss0
...
CONTAINS
   ...
   INTEGER FUNCTION zdf_tke_alloc()
      ...
      !!----------------------------------------------------------------------
      ALLOCATE( htau(jpi,jpj) , dissl(jpi,jpj,jpk) , apdlr(jpi,jpj,jpk) , ztmp0(jpi,jpj), rn_ediss0(jpi,jpj), STAT= zdf_tke_alloc )
      !
  ...
  END SUBROUTINE zdf_tke
   ...
   SUBROUTINE tke_tke( pdepw, p_e3t, p_e3w, p_sh2, p_avm, p_avt )
      ...
      REAL(wp) ::   zbbirau, zri       ! local scalars
      REAL(wp) ::   zfact1
      ...
      REAL(wp), DIMENSION(jpi,jpj)     ::   zbbrau, zfact2, zfact3
      ...
      IF( nn_spp_tkebb > 0 ) THEN
          CALL spp_gen(0,zbbrau,nn_spp_tkebb,rn_tkebb_sd,   jk_spp_tkebb )
      ENDIF
...
!!             &                                     fr_i(ji,jj)   * zbbirau ) * taum(ji,jj) ) * tmask(ji,jj,1)
      ...
      IF( ln_lc ) THEN      !  Langmuir circulation source term added to tke   !   (Axell JGR 2002)
         ...
         IF( nn_spp_tkelc > 0 ) THEN
           CALL spp_gen(0,ztmp0,nn_spp_tkelc,rn_tkelc_sd,    jk_spp_tkelc )
         ENDIF
               ...
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  IF ( zus3(ji,jj) /= 0._wp ) THEN               
                     ...
                     IF ( pdepw(ji,jj,jk) - zhlc(ji,jj) < 0 .AND. wmask(ji,jj,jk) /= 0. ) THEN
                        !                                           ! vertical velocity due to LC
                        zwlc = ztmp0(ji,jj) * SIN( rpi * pdepw(ji,jj,jk) / zhlc(ji,jj) )
      ...
      DO jk = 2, jpkm1           !* Matrix and right hand side in en
         DO jj = 2, jpjm1
            DO ji = fs_2, fs_jpim1   ! vector opt.
               ...
               zdiag(ji,jj,jk) = 1._wp - zzd_lw - zzd_up + zfact2(ji,jj) * dissl(ji,jj,jk) * wmask(ji,jj,jk)
               !
               !                                   ! right hand side in en
               en(ji,jj,jk) = en(ji,jj,jk) + rdt * (  p_sh2(ji,jj,jk)                          &   ! shear
                  &                                 - p_avt(ji,jj,jk) * rn2(ji,jj,jk)          &   ! stratification
                  &                                 + zfact3(ji,jj) * dissl(ji,jj,jk) * en(ji,jj,jk)  &   ! dissipation
                  &                                ) * wmask(ji,jj,jk)
            END DO
         END DO
      END DO
...
!!gm       i.e. use gde3w in argument (pdepw)
      ...
      IF( nn_spp_tkefr > 0 ) THEN
          CALL spp_gen( 0,ztmp0,nn_spp_tkefr,rn_tkefr_sd,   jk_spp_tkefr )
      ENDIF
      IF( nn_etau == 1 ) THEN           !* penetration below the mixed layer (rn_efr fraction)
         DO jk = 2, jpkm1                       ! nn_eice=0 : ON below sea-ice ; nn_eice>0 : partly OFF
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  en(ji,jj,jk) = en(ji,jj,jk) + ztmp0(ji,jj) * en(ji,jj,1) * EXP( -pdepw(ji,jj,jk) / htau(ji,jj) )   &
         ...
         DO jk = 2, jpkm1
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  ...
                  en(ji,jj,jk) = en(ji,jj,jk) + zbbrau(ji,jj) * zdif * EXP( -pdepw(ji,jj,jk) / htau(ji,jj) )   &
                     &                                 * MAX( 0._wp, 1._wp - zice_fra(ji,jj) ) * wmask(ji,jj,jk) * tmask(ji,jj,1)
               END DO
            END DO
         END DO
 ...
 !!gm Not sure of that coding for ISF....
      ...
      IF( nn_spp_tkedf > 0 ) THEN
          CALL spp_gen(0,ztmp0,nn_spp_tkedf,rn_tkedf_sd, jk_spp_tkedf )
      ENDIF
      DO jk = 1, jpkm1            !* vertical eddy viscosity & diffivity at w-points
         DO jj = 2, jpjm1
            DO ji = fs_2, fs_jpim1   ! vector opt.
               zsqen = SQRT( en(ji,jj,jk) )
               zav   = ztmp0(ji,jj) * zmxlm(ji,jj,jk) * zsqen
      ...
      ENDIF
      DO jk = 1, jpkm1            !* vertical eddy viscosity & diffivity at w-points
         IF(nn_spp_avt > 0 ) CALL spp_gen(1 ,p_avt(:,:,jk),nn_spp_avt,rn_avt_sd, jk_spp_avt, jk)
         IF(nn_spp_avm > 0 ) CALL spp_gen(1 ,p_avm(:,:,jk),nn_spp_avm,rn_avm_sd, jk_spp_avm, jk)
      ENDDO
      !
      IF(ln_ctl) THEN
