...
!!JC: possible WAD implementation should modify line below if layers vanish
   ...
   SUBROUTINE zdf_drg_exp( kt, pub, pvb, pua, pva )
      ...
      REAL(wp), DIMENSION(:,:  ), ALLOCATABLE ::   rCdU_bot0
      !!---------------------------------------------------------------------
      !
!!gm bug : time step is only rdt (not 2 rdt if euler start !)
      ...
      IF( l_trddyn ) THEN      ! trends: store the input trends
         ...
         ztrdv(:,:,:) = pva(:,:,:)
      ENDIF
      ALLOCATE(rCdU_bot0(jpi,jpj))
      rCdU_bot0(:,:) = rCdU_bot(:,:)
      IF( nn_spp_bfr > 0 ) THEN
         CALL spp_gen(kt, rCdU_bot0, nn_spp_bfr, rn_bfr_sd, jk_spp_bfr)
      ENDIF
      ...
      DO jj = 2, jpjm1
         DO ji = 2, jpim1
            ...
            zCdu = 0.5*( rCdU_bot0(ji+1,jj)+rCdU_bot0(ji,jj) ) / e3u_n(ji,jj,ikbu)
            zCdv = 0.5*( rCdU_bot0(ji,jj+1)+rCdU_bot0(ji,jj) ) / e3v_n(ji,jj,ikbv)
            !
