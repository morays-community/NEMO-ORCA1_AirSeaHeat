...
!$AGRIF_DO_NOT_TREAT
   ...
   INTEGER, PUBLIC ::   mpi_comm_all   ! opa local communicator
!$AGRIF_END_DO_NOT_TREAT
   ...
   LOGICAL, PUBLIC                                  :: ln_ensemble        = .FALSE.
   INTEGER, PUBLIC                                  :: nn_ens_size        = 1
   INTEGER, PUBLIC                                  :: nn_ens_start       = 1
   INTEGER, DIMENSION(:), ALLOCATABLE, SAVE         :: ncomm_member   ! communicator for every ens member
   INTEGER, PUBLIC                                  :: nmember 
   CHARACTER(len=3), PUBLIC                         :: c_nmem     !: character string with ensemble member
   CHARACTER(len=4), PUBLIC                         :: c_enss     !: character string with ensemble member
...
CONTAINS
   ...
   SUBROUTINE mpp_start( localComm )
      ...
      IF( PRESENT(localComm) ) THEN
         ...
         ENDIF
      ELSE
         CALL mpi_comm_dup( mpi_comm_world, mpi_comm_all, ierr)
      ...
      IF( ln_ensemble ) THEN
              ...
              WRITE(c_enss,'(A1,I3.3)') '.',nmember
      ELSE
              c_enss = ''
      ENDIF

# if defined key_agrif
      IF (ln_ensemble) CALL ctl_stop( 'STOP', ' lib_mpp: Ens not supported with AGRIF' )
      IF( Agrif_Root() ) THEN
...
# endif
      ...
      mppsize = 1
      mpprank = 0
      IF (ln_ensemble) CALL ctl_stop( 'STOP', ' lib_mpp: Ens not supported without key_mpp_mpi' )
#endif
...
#  undef OPERATION_MAXLOC
   ...
   SUBROUTINE ctl_stop( cd1, cd2, cd3, cd4, cd5 ,   &
      ...
      IF( cd1 == 'STOP' .AND. narea /= 1 ) THEN    ! Immediate stop: add an arror message in 'ocean.output' file
         CALL ctl_opn( inum, 'ocean.output'//TRIM(c_enss), 'APPEND', 'FORMATTED', 'SEQUENTIAL', -1, 6, .FALSE. )
         WRITE(inum,*)
      ...
      IF( numout == 6 ) THEN                       ! force to open ocean.output file if not already opened
         CALL ctl_opn( numout, 'ocean.output'//TRIM(c_enss), 'REPLACE', 'FORMATTED', 'SEQUENTIAL', -1, -1, .FALSE., narea )
      ENDIF
      !
                            WRITE(numout,*)
...
100   CONTINUE
   ...
   !!----------------------------------------------------------------------
      ...
      !!----------------------------------------------------------------------
      CALL mpi_comm_size( klocalcomm , jpnij , ierr )
      jpnij = jpnij / nn_ens_size
      WRITE(*,*) ' TOTAL Procs = ',jpnij
      ...
      mpi_comm_oce = ncomm_member(imember)
      ! Deallocate arrays
...
!      ENDIF
   ...
   SUBROUTINE mpp_ens_init
      !!----------------------------------------------------------------------
      !!                  ***  routine mpp_ens_init  ***
      !!
      !! ** Purpose :   Set up ensemble size of NEMO through env. variable
      !!
      !! ** Method  :   Read environmental variable "nemo_num_members"
      !!----------------------------------------------------------------------
      INTEGER ::   ienss, iaux   ! local integers
      CHARACTER(LEN=99) :: caux
      !!----------------------------------------------------------------------
      !
      CALL get_environment_variable("nemo_num_members", caux)
      IF(len_trim(caux) > 0) THEN
              READ(caux,*, iostat=iaux) ienss
              IF(iaux.ne.0) THEN
                      WRITE(*,*) ' ERROR in reading Ensemble Size '
                      CALL ABORT()
              ENDIF
              IF(ienss>1 .AND. ienss<999) THEN
                      WRITE(*,*) ' ENSEMBLE CONFIGURATION, SIZE = ',ienss
                      nn_ens_size = ienss
                      ln_ensemble = .TRUE.
              ENDIF
      ENDIF
      !
   END SUBROUTINE mpp_ens_init
