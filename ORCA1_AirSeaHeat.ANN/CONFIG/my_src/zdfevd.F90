MODULE zdfevd
   ...
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:) :: zn_evd0
...
CONTAINS
   ...
   SUBROUTINE zdf_evd( kt, p_avm, p_avt )
      ...
      !
      IF(ln_stopack .AND. nn_spp_aevd.GT.0) THEN
         zn_evd0(:,:) = rn_evd
         CALL spp_gen(kt, zn_evd0, nn_spp_aevd, rn_aevd_sd, jk_spp_aevd)
      ENDIF
      !
      SELECT CASE ( nn_evdm )
      !
            ...
            DO jj = 2, jpjm1
               DO ji = 2, jpim1
                  IF(  MIN( rn2(ji,jj,jk), rn2b(ji,jj,jk) ) <= -1.e-12 ) THEN
                     p_avt(ji,jj,jk) = zn_evd0(ji,jj) * wmask(ji,jj,jk)
                     p_avm(ji,jj,jk) = zn_evd0(ji,jj) * wmask(ji,jj,jk)
                  ENDIF
               END DO
            END DO
         END DO 
