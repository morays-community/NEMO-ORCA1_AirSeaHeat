...
CONTAINS
   ...
   SUBROUTINE ice_thd_zdf_init
      ...
      NAMELIST/namthd_zdf/ ln_zdf_BL99, ln_cndi_U64, ln_cndi_P07, rn_cnd_s, &
         &                 rn_kappa_i, rn_kappa_s, rn_kappa_smlt, rn_kappa_sdry, ln_zdf_chkcvg, rn_oiht
      !!-------------------------------------------------------------------
      !
      rn_oiht = 0.0057
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namthd_zdf in configuration namelist' )
      ...
      IF(lwp) THEN                          ! control print
         ...
         WRITE(numout,*) '      heat flux from ocean to sea ice                           rn_oiht       = ', rn_oiht
      ...
      ENDIF
      IF ( rn_oiht .lt. 0.001 .or. rn_oiht .gt. 0.01 ) CALL ctl_stop( 'ice_thd_zdf_init: rn_oiht is not set')
...
!!    IF( ln_zdf_XXXX ) THEN   ;   ioptio = ioptio + 1   ;   nice_zdf = np_XXXX   ;   ENDIF
      ...
      !
      ALLOCATE (rn_cnd_s2d(jpi,jpj)) ; rn_cnd_s2d(:,:) = rn_cnd_s
      !
