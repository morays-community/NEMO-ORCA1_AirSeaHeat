            ...
            IF( h_i_1d(ji) > 0._wp ) THEN
               ...
               IF( ld_sal ) THEN
                  IF( t_su_1d(ji) >= rt0 ) THEN             ! flushing (summer time)
                     zds = - MAX( s_i_1d(ji) - rn_sal_fl , 0._wp ) * rdt_ice / rn_time_fl1d(ji)
                  ELSEIF( t_su_1d(ji) <= t_bo_1d(ji) ) THEN ! gravity drainage
                     zds = - MAX( s_i_1d(ji) - rn_sal_gd , 0._wp ) * rdt_ice / rn_time_gd1d(ji)
                  ELSE
                     zds = 0._wp
                  ENDIF
                  ! update salinity
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namthd_sal in configuration namelist' )
      ...
      ENDIF
      !
      ALLOCATE (rn_time_fl2d(jpi,jpj)) ; rn_time_fl2d(:,:) = rn_time_fl
      ALLOCATE (rn_time_gd2d(jpi,jpj)) ; rn_time_gd2d(:,:) = rn_time_gd
      !
