      ...
      DO WHILE ( ( .NOT. ALL (l_T_converged(1:npti)) ) .AND. iconv < iconv_max )   ! Iterative procedure begins !
         ...
         IF( ln_virtual_itd ) THEN
            ...
            DO ji = 1, npti
               ...
               zhe = ( rn_cnd_s1d(ji) * h_i_1d(ji) + zcnd_i * h_s_1d(ji) ) / ( rn_cnd_s1d(ji) + zcnd_i )
         ...
         DO jk = 0, nlay_s-1
            DO ji = 1, npti
               IF ( .NOT. l_T_converged(ji) ) &
                  zkappa_s(ji,jk) = zghe(ji) * rn_cnd_s1d(ji) * z1_h_s(ji)
            END DO
         END DO
         DO ji = 1, npti   ! Snow-ice interface
            IF ( .NOT. l_T_converged(ji) ) &
               zkappa_s(ji,nlay_s) = isnow(ji) * zghe(ji) * rn_cnd_s1d(ji) * ztcond_i(ji,0) &
                  &                            / ( 0.5_wp * ( ztcond_i(ji,0) * zh_s(ji) + rn_cnd_s1d(ji) * zh_i(ji) ) )
         END DO
...
!!$            END DO
      ...
      DO ji = 1, npti         
         ...
         IF( h_s_1d(ji) >= zhs_ssl ) THEN
            t_si_1d(ji) = (   rn_cnd_s1d(ji) * h_i_1d(ji) * r1_nlay_i * t_s_1d(ji,nlay_s)   &
               &            + ztcond_i(ji,1) * h_s_1d(ji) * r1_nlay_s * t_i_1d(ji,1)      ) &
               &          / ( rn_cnd_s1d(ji) * h_i_1d(ji) * r1_nlay_i &
