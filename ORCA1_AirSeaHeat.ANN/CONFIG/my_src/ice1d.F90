MODULE ice1D
   ...
   ! for SPP routines
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:) ::   rn_cnd_s1d
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:) ::   rn_pnd_flush1d
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:) ::   rn_time_fl1d
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:) ::   rn_time_gd1d
...
CONTAINS
   ...
   FUNCTION ice1D_alloc()
      ...
      !
      ii = ii + 1
      ALLOCATE( sst_1d(jpij) , sss_1d(jpij) , frq_m_1d(jpij) , rn_cnd_s1d(jpij), &
         &      rn_pnd_flush1d(jpij), rn_time_fl1d(jpij), rn_time_gd1d(jpij), STAT=ierr(ii) )
      !
      ii = ii + 1
