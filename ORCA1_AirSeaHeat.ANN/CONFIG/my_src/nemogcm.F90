...
# endif
   ...
   SUBROUTINE nemo_init
      ...
      IF( lwm )   CALL ctl_opn(     numout,        'ocean.output'//TRIM(c_enss), 'REPLACE', 'FORMATTED', 'SEQUENTIAL', -1, -1, .FALSE. )
      ! open reference and configuration namelist files
                  CALL ctl_opn( numnam_ref,        'namelist_ref',     'OLD', 'FORMATTED', 'SEQUENTIAL', -1, -1, .FALSE. )
                  CALL ctl_opn( numnam_cfg,        'namelist_cfg'//TRIM(c_enss),     'OLD', 'FORMATTED', 'SEQUENTIAL', -1, -1, .FALSE. )
      IF( lwm )   CALL ctl_opn(     numond, 'output.namelist.dyn'//TRIM(c_enss), 'REPLACE', 'FORMATTED', 'SEQUENTIAL', -1, -1, .FALSE. )
